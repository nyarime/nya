...
`,
		...
		"send.tunnel.origincert": "nya send: cloudflared needs an origin certificate from a named tunnel setup, not a TryCloudflare quick tunnel.",
		"send.tunnel.origincert.hint": "  tip: unset TUNNEL_ORIGIN_CERT if empty; rename ~/.cloudflared/config.yml if you only need quick share; or use nya send -no-tunnel for LAN only",
...
`,
		...
		"send.tunnel.origincert": "nya send: cloudflared 在找命名隧道的 origin 证书，而不是 TryCloudflare 临时隧道。",
		"send.tunnel.origincert.hint": "  提示：若 TUNNEL_ORIGIN_CERT 为空请取消设置；仅需临时分享可重命名 ~/.cloudflared/config.yml；或 nya send -no-tunnel 仅局域网",
